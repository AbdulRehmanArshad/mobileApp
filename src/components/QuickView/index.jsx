...
import { slice as QuickValue } from "../../store/cartProduct/index"

const QuickView = (props) => {
    const dispatch = useDispatch()
    const viewItems = useSelector(state => state.cart)
    const [viewstate] = useState(viewItems.view)
    const HanldeClick = () => {
        console.log("🚀 ~ file: index.jsx ~ line 8 ~ QuickView ~ viewItems", viewstate.id)

    }
    const ViewValueCart = () => {
        dispatch(QuickValue.actions.allQuickvalue(viewItems.view))
    }
    return (
        <>
            <button onClick={HanldeClick}>hello</button>
            <div className="bg-gray-800">
                <div className="2xl:container 2xl:mx-auto md:py-12 lg:px-20 md:px-6 py-9 px-4">

                    <div id="viewerBox" className="lg:p-10 md:p-6 p-4 bg-white">
                        <div className="flex justify-end">
                            <Link to="/apple-mobile">
                        ...
                        <div className="mt-3 md:mt-4 lg:mt-0 flex flex-col lg:flex-row items-stretch justify-center lg:space-x-8">
                            ...
                            <div className="slide-ana lg:relative">
                                <img style={{ width: "250px", height: "250px" }} src={viewstate.img} alt="A black chair with wooden legs" className="w-full h-full" />


                            </div>

                            <div className="lg:w-1/2 flex flex-col justify-center mt-7 md:mt-8 lg:mt-0 pb-8 lg:pb-0">
                                <h1 className="text-3xl lg:text-4xl font-semibold text-gray-800">{viewstate.tittle}</h1>
                                ...
                                <p className="text-3xl font-medium text-gray-600 mt-8 md:mt-10">{`Rs: ${viewstate.Price}`}</p>
                                <div className="flex items-center flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6 lg:space-x-8 mt-8 md:mt-16">
                                    <button
                                        onClick={ViewValueCart}
                                        className="w-full md:w-3/5 border border-gray-800 text-base font-medium leading-none text-white uppercase py-6 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 bg-gray-800 text-white">Add to Cart</button>
                                </div>
