...
const Cart = () => {
  ...
  const Increasevalue = (item, index) => {
    ...
    const arr = cartItemState.map((e) => {
      return e.id === item.id
        ? { ...e, quantity: e.quantity + 1, Price: e.Price + total }
        : e;
    });
  ...
  const Decreasevalue = (item, index) => {
    ...
    let arr = cartItemState.map((_) => {
      return _.id === item.id
        ? { ..._, quantity: _.quantity - 1, Price: _.Price - total }
        : _;
    });
    // setCartItemState(arr);
    console.log(arr);
  ...
  return (
    <>
      <Header quantity={totalquantityvalue} />
      {IsShow ? <img style={{ width: "100%", height: "400px" }} src="https://mir-s3-cdn-cf.behance.net/projects/404/95974e121862329.Y3JvcCw5MjIsNzIxLDAsMTM5.png" /> : <> <div className="container p-8 mx-auto mt-12">
        <div className="w-full overflow-x-auto">
          <div className="my-2">
            <h3 className="text-xl font-bold tracking-wider">
              Shopping Cart {totalquantityvalue} item
            </h3>
          </div>
